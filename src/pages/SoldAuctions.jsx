...
export default function SoldAuctions() {
  const [soldAuctions, setSoldAuctions] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const { user } = useContext(Context);
  const [mockSoldAuctions, setMockSoldAuctions] = useState(AS);
  const [filteredAuctions, setFilteredAuctions] = useState([]);

  useEffect(() => {
    document.title = "S&D - Sold Auctions";
    setTimeout(() => {
      setSoldAuctions(mockSoldAuctions);
    }, 500);

    const fetchSoldAuctions = async () => {
      const res = await fetch(`/api/transaction/sold/user/${user.id}`, {
        method: "GET",
        headers: {
